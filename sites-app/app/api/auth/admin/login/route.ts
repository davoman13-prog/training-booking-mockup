...
import { createAdminSession, verifyPassword } from "../../auth";
...
export async function POST(request: Request) {
  try {
    ...
    if (account.role !== "Admin" || !account.is_active) return Response.json({ code: "ACCOUNT_UNAVAILABLE", message: "This account is not currently active." }, { status: 403 });
    ...
    return Response.json(
      { user: { id: account.id, name: `${account.first_name} ${account.last_name}`.trim(), email: account.email, role: "admin" } },
